...
function generateFixedPanels(input: CabinetUnitInput, unitQty: number): PanelResult[] {
  ...
  const leftZhengdaoProcesses = isZhengdao ? zhengdaoProcessingRows({
    idPrefix: `${id}-left`,
    singleCm2: sidePanelWidthCm * depthCm,
    totalQty: unitQty,
    processes: panelProcesses.left.zhengdaoProcesses,
  }) : [];
  const rightZhengdaoProcesses = isZhengdao ? zhengdaoProcessingRows({
    idPrefix: `${id}-right`,
    singleCm2: sidePanelWidthCm * depthCm,
    totalQty: unitQty,
    processes: panelProcesses.right.zhengdaoProcesses,
  }) : [];
  const topZhengdaoProcesses = isZhengdao ? zhengdaoProcessingRows({
    idPrefix: `${id}-top`,
    singleCm2: topWidthCm * topDepthCm,
    totalQty: unitQty,
    processes: panelProcesses.top.zhengdaoProcesses,
  }) : [];
  const bottomZhengdaoProcesses = isZhengdao ? zhengdaoProcessingRows({
    idPrefix: `${id}-bottom`,
    singleCm2: bottomWidthCm * depthCm,
    totalQty: unitQty,
    processes: panelProcesses.bottom.zhengdaoProcesses,
  }) : [];
  const leftZhengdaoProcessCost = leftZhengdaoProcesses.reduce((sum, process) => sum + process.cost, 0);
  const rightZhengdaoProcessCost = rightZhengdaoProcesses.reduce((sum, process) => sum + process.cost, 0);
  const topZhengdaoProcessCost = topZhengdaoProcesses.reduce((sum, process) => sum + process.cost, 0);
  const bottomZhengdaoProcessCost = bottomZhengdaoProcesses.reduce((sum, process) => sum + process.cost, 0);
  ...
  for (const sealPanel of input.sideTopBottomSealPanels ?? []) {
    const sealPanelQuantity = sealPanel.quantity * unitQty;
    const sealZhengdaoProcesses = isZhengdao
      ? zhengdaoProcessingRows({
          idPrefix: sealPanel.id,
          singleCm2: sealPanel.widthCm * sealPanel.heightCm,
          totalQty: sealPanelQuantity,
          processes: sealPanel.zhengdaoProcesses,
        })
      : [];
    const sealZhengdaoProcessCost = sealZhengdaoProcesses.reduce((sum, process) => sum + process.cost, 0);
    panels.push(
      buildPanelResult({
        id: sealPanel.id,
        ...
        quantity: sealPanelQuantity,
        materialRef: sealPanel.materialRef,
        isAutoGenerated: true,
        addonPricePerCai: isZhengdao ? 0 : frontEdgeAddon,
        extraAddonCost: sealZhengdaoProcessCost,
        processes: sealZhengdaoProcesses,
      }),
    );
  }
...
function generateInternalParts(input: CabinetUnitInput, unitQty: number): PanelResult[] {
  ...
  for (const d of input.middleDividers) {
    ...
    const dividerZhengdaoProcesses = vendor === "ZHENGDAO"
      ? zhengdaoProcessingRows({
          idPrefix: d.id,
          singleCm2: dividerWidthCm * dividerHeightCm,
          totalQty: dividerQuantity,
          processes: d.zhengdaoProcesses,
        })
      : [];
    const dividerZhengdaoProcessCost = dividerZhengdaoProcesses.reduce((sum, process) => sum + process.cost, 0);
    parts.push(buildPanelResult({
      id: d.id,
      name: "中立板",
      ...
      isAutoGenerated: false,
      addonPricePerCai: frontEdgeAddon,
      extraAddonCost: dividerLightGrooveCost + dividerSpecialCost + dividerHoleCost + dividerHardwareCost + dividerZhengdaoProcessCost,
  ...
  for (const s of input.shelves) {
    ...
    const shelfZhengdaoProcesses = vendor === "ZHENGDAO"
      ? zhengdaoProcessingRows({
          idPrefix: s.id,
          singleCm2: s.widthCm * shelfDepthCm,
          totalQty: shelfQuantity,
          processes: s.zhengdaoProcesses,
        })
      : [];
    const shelfZhengdaoProcessCost = shelfZhengdaoProcesses.reduce((sum, process) => sum + process.cost, 0);
    ...
    parts.push(buildPanelResult({
      ...
      isAutoGenerated: false,
      addonPricePerCai: frontEdgeAddon,
      extraAddonCost: shelfLightGrooveCost + shelfSpecialCost + shelfHardwareCost + shelfZhengdaoProcessCost,
  ...
  for (const tray of input.drawerTrays ?? []) {
    drawerParts.push(buildPanelResult({
      id: `${tray.id}-tray-panel`,
      name: tray.name.trim() || "抽盤",
      widthCm: tray.widthCm,
      heightCm: tray.depthCm,
      quantity: tray.quantity * unitQty,
      materialRef: tray.materialRef,
      isAutoGenerated: false,
      billableMinCai: null,
    }));

    if (tray.frontPanel?.enabled) {
      drawerParts.push(buildPanelResult({
        id: `${tray.id}-front-panel`,
        name: `${tray.name.trim() || "抽盤"}-抽頭`,
        widthCm: tray.frontPanel.widthCm,
        heightCm: tray.frontPanel.heightCm,
        quantity: tray.quantity * unitQty,
        materialRef: tray.frontPanel.materialRef,
        isAutoGenerated: false,
        billableMinCai: null,
      }));
    }
...
function calculateDrawerHardware(input: CabinetUnitInput, unitQty: number): HardwareResult[] {
  const drawerRails = (input.drawers ?? []).filter((drawer) => drawer.includeRailInQuote ?? true).map((drawer) => {
    const quantity = (drawer.railQuantity ?? drawer.quantity) * unitQty;
  ...
  const trayRails = (input.drawerTrays ?? []).filter((tray) => tray.includeRailInQuote ?? false).map((tray) => {
    const quantity = (tray.railQuantity ?? tray.quantity) * unitQty;
    const railRef = tray.railMaterialRef ?? null;
    return {
      id: `${tray.id}-rail`,
      name: "抽盤滑軌",
      description: `${tray.name || "抽盤"} ${tray.widthCm}×${tray.depthCm}cm × ${quantity}`,
      quantity,
      materialRef: railRef,
      unitCost: railRef?.pricePerUnit ?? 0,
      subtotal: railRef ? round(quantity * railRef.pricePerUnit, UNIT_CONFIG.COST_DECIMAL_PLACES) : 0,
...
function calculateOtherCosts(items: OtherCostItemInput[] | undefined, unitQty: number): OtherCostResult[] {
  return (items ?? []).map((item) => {
    const quantity = item.quantity * unitQty;
    const unitCost = item.unitPrice;
    return {
      id: `${item.id}-other-cost`,
      name: item.name || "其他費用",
      description: "",
      quantity,
      unitCost,
...
function zhengdaoProcessingRows(params: {
  idPrefix: string;
  singleCm2: number;
  totalQty: number;
  processes: import("@/types/zhengdao-door").ZhengdaoDoorProcessInput[] | undefined;
}): PanelProcessResult[] {
  return (params.processes ?? []).flatMap((processInput) => {
    const rule = ZHENGDAO_2025_DOOR_PROCESS_RULE_BY_CODE[processInput.code];
    if (!rule || processInput.quantityPerDoor <= 0) return [];

    let quantity = processInput.quantityPerDoor * params.totalQty;
    if (rule.billingMode === "PER_CAI") {
      quantity = toBillableAreaMeasure(params.singleCm2, rule.minCai ?? null, params.totalQty).cai * processInput.quantityPerDoor;
    } else if (rule.billingMode === "PER_10MM") {
      quantity = Math.ceil(Math.max(processInput.lengthMm ?? 0, 0) / 10) * processInput.quantityPerDoor * params.totalQty;
    }
    ...
    return [panelProcess(`${params.idPrefix}-${processInput.id}`, rule.name, cost, true, quantity, rule.unitPrice)];
  });
}
  ...
  for (const door of input.doors) {
    ...
    const zhengdaoProcesses = isZhengdao
      ? zhengdaoProcessingRows({
          ...
          singleCm2,
          totalQty,
          processes: door.zhengdaoProcesses,
        })
      : [];
...
function buildSummary(
  ...
  const otherCost = otherCosts.reduce((acc, item) => acc + item.subtotal, 0);
  ...
  return {
    ...
    addonsBreakdown,
    totalCost: panelsCost + internalPartsCost + doorsCost + hardwareCost + otherCost + accessoriesCost + backPanelGroove,
  };
}
...
export function calculateCabinetUnit(input: CabinetUnitInput): CabinetUnitResult {
  ...
  const otherCosts = calculateOtherCosts(input.otherCostItems, unitQty);
  const accessories = calculateAccessories(input, unitQty);
  const summary = buildSummary(panels, internalParts, doors, hardware, otherCosts, accessories, addonsBreakdown);
