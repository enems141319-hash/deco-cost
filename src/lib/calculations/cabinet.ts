...
function lTurnCabinetPanels(input: CabinetUnitInput, unitQty: number): PanelResult[] {
  ...
  const backPanelGrooveNote = input.hasBackPanel ? buildBackPanelGrooveNote() : undefined;
  const backPanelGrooveCostPerLine = input.hasBackPanel ? UNIT_CONFIG.BACK_PANEL_GROOVE_COST_PER_LINE : 0;
...
function generateFixedPanels(input: CabinetUnitInput, unitQty: number): PanelResult[] {
  ...
  if (addons.lTurnCabinet?.enabled) {
    ...
    const normalPanelIds = new Set([`${id}-left`, `${id}-right`, `${id}-back`]);
...
function generateInternalParts(input: CabinetUnitInput, unitQty: number): PanelResult[] {
  ...
  const sideThicknessCm = materialThicknessCm(input.panelMaterialRef);
  const autoDrawerDividerHeightCm = round(
    Math.max(input.heightCm - sideThicknessCm * 2 - (input.kickPlate?.heightCm ?? 0), 0),
    UNIT_CONFIG.DIMENSION_DECIMAL_PLACES,
  );
  ...
  for (const d of input.middleDividers) {
    ...
    const dividerHoleProcesses = middleDividerHoleProcesses(d.id, d.widthCm, d.heightCm, dividerQuantity, d.materialRef, d.addons);
    ...
    parts.push(buildPanelResult({
      ...
      processes: [
        ...
        ...(dividerLightGrooveNote
          ? [lightGrooveProcess(`${d.id}-light-groove`, dividerLightGrooveNote, d.heightCm, dividerQuantity)]
          : []),
    ...
    if (door.type === "SLIDING" && (door.includeSlidingHardwareInQuote ?? true)) {
      ...
      if (railRef) {
        ...
        hardware.push({
          ...
          quantity,
          materialRef: railRef,
          unitCost: railRef.pricePerUnit,
          subtotal: round(quantity * railRef.pricePerUnit, UNIT_CONFIG.COST_DECIMAL_PLACES),
        });
      }
    }
