  ...
  return (
    <div className="min-w-0 space-y-1">
      <div className="flex min-w-0 items-center gap-1.5">
        <span className="shrink-0 text-sm font-semibold text-slate-800">{displayIndex}</span>
        ...
        {"type" in row && (
          <Badge variant="secondary" className="shrink-0 px-1 py-0 text-[10px]">
            {row.isLouverDoor ? "格柵門" : row.type === "HINGED" ? "鉸鏈門" : "滑門"}
          </Badge>
        )}
      </div>
          ...
          <tbody>
            {rows.map((hardware, rowIndex) => (
              <tr key={hardware.id} className="border-b border-muted/50 hover:bg-muted/20">
                <td className={`${bodyCellClass} min-w-0`}>
                  <div className="flex min-w-0 items-center gap-1.5">
                    <span className="shrink-0 font-medium text-muted-foreground">{sectionNumber}-{rowIndex + 1}</span>
                    <span className="truncate font-medium">{hardware.materialRef?.materialName ?? hardware.name}</span>
                  </div>
                  <div className="truncate text-xs text-muted-foreground">
                  ...
                  {result.accessories.map((accessory, rowIndex) => (
                    <tr key={accessory.id} className="border-b border-muted/50 hover:bg-muted/20">
                      <td className={`${bodyCellClass} font-medium`}>
                        <div className="flex min-w-0 items-center gap-1.5">
                          <span className="shrink-0 text-muted-foreground">{accessoriesSection}-{rowIndex + 1}</span>
